...
for (my $i=1; $i<=$num_buildings; $i++){
	for (my $j=1; $j<=$num_rooms[$i-1]; $j++){
		...
		print 'INSERT INTO `rooms` (`room_num`,`build_id`,`group_id`,`num_students`,`floor`,`gender`,`smoking`,`RA_Room`,`HC_Room`)';
		print 'VALUES ('."$j".',"'.$i.'","0","0",'.$floor.','.$gender.','.$smoking.',0,0);' . "\n";
...
for (my $i=1; $i<=$num_students; $i++){
	...
	print '`others_drink`,`smoking`,`others_smoking`,`noise_rating`,`cleanliness_rating`,`lifestyle_rating`,`age_rating`,`major_rating`,`guest_rating`,';
	print '`comments`,`comments_resolved`,`req_room_num`,`req_bedroom_letter`,`req_build_id`)'; 
